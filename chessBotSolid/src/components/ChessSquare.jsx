...
function ChessSquare({
  ...
  function findPieceMoves(index) {
    ...
    if (
      chessBoard()[index] === blackRook ||
      chessBoard()[index] === whiteRook ||
      chessBoard()[index] === "T" ||
      chessBoard()[index] === "t"
    ) {
      ...
      if (rookMoves != undefined)
        if (rookMoves.length > 0) moves.push(...rookMoves);
    }
    //biship moves
    if (
      chessBoard()[index] === blackBishop ||
      chessBoard()[index] === whiteBishop
    ) {
      moves.push(...findBishopMoves(chessBoard(), index));
    }
    //knight moves
    if (
      chessBoard()[index] === blackKnight ||
      chessBoard()[index] === whiteKnight
    ) {
      moves.push(...findHorseMoves(chessBoard(), index));
    }
    //queen moves
    if (
      chessBoard()[index] === blackQueen ||
      chessBoard()[index] === whiteQueen
    ) {
      moves.push(...findQueenMoves(chessBoard(), index));
    }
    //king moves
    if (
      chessBoard()[index] === blackKing ||
      chessBoard()[index] === whiteKing
    ) {
      moves.push(...findKingMoves(chessBoard(), index, moves));
    }
    if (
      chessBoard()[index] === blackPawn ||
      chessBoard()[index] === whitePawn
    ) {
  ...
  const Draggable = ({ id, index, className }) => {
    ...
    const [, { onDragEnd, onDragStart, activeDraggable }] =
      useDragDropContext();
    // console.log(index);
    onDragEnd(({ draggable }) => {
      ...
      if (litUpBoxes()[hoveredElement()] === "circle") {
        if (index === hoveredElement() && activeItem() != null) {
          ...
          if (
            chessBoard()[CurrentIndex] === whitePawn ||
            chessBoard()[CurrentIndex] === blackPawn
          ) {
            console.log("piece is a pawn");
            if (
              (NewIndex - CurrentIndex != 8 || NewIndex - CurrentIndex != -8) &&
              newChessBoard[NewIndex] === emptySpace
            ) {
              if (
                chessBoard()[CurrentIndex] === blackPawn &&
          ...
          if (activeItem().getAttribute("index") - 0 != hoveredElement()) {
            newChessBoard[activeItem().getAttribute("index") - 0] = "-";
            lastMove = [
              activeItem().getAttribute("index") - 0,
              hoveredElement() - activeItem().getAttribute("index"),
            ];
            setPreviousMove(lastMove);
          }

          if (
            chessBoard()[NewIndex] === whiteKing ||
            chessBoard()[NewIndex] === blackKing
          ) {
            if (
              CurrentIndex - NewIndex === 2 ||
              CurrentIndex - NewIndex === -2
            ) {
              if (NewIndex === 58) {
                newChessBoard[56] = "-";
        ...
        if (
          index === activeItem().getAttribute("index") - 0 &&
          activeItem().getAttribute("index") != hoveredElement()
        ) {
          let newPieceClass = pieceClass();
    ...
    return (
      <div
        ...
        use:droppable
      >
        <section use:draggable class={`${pieceClass()[index]}`} index={index}>
    ...
    if (chessBoard[index] === blackPawn && index < ONEROWFROMBOTTOM) {
      ...
      if (index >= 32 && index <= 39) {
        ...
        if (chessBoard[index + right] === whitePawn) {
          if (
            previousMove()[0] + previousMove()[1] === index + 1 &&
            previousMove()[1] === -16
          ) {
            moves.push(index);
      ...
      if (
        chessBoard[index + rightDown] !== emptySpace &&
        chessBoard[index + rightDown].charCodeAt() < COLORDELIMITER
      ) {
        moves.push(index);
        moves.push(rightDown);
      }
      if (
        chessBoard[index + leftDown] !== emptySpace &&
        chessBoard[index + leftDown].charCodeAt() < COLORDELIMITER
      ) {
        moves.push(index);
        moves.push(leftDown);
      }
      if (
        index <= 15 &&
        index >= 8 &&
        chessBoard[index + spaceDown * 2] === emptySpace
      ) {
        moves.push(index);
    ...
    if (chessBoard[index] === whitePawn && index >= ONEROWFROMTOP) {
      ...
      if (index >= 24 && index <= 31) {
        ...
        if (chessBoard[index + right] === blackPawn) {
          if (
            previousMove()[0] + previousMove()[1] === index + right &&
            previousMove()[1] === 16
          ) {
            moves.push(index);
            moves.push(rightUp);
        ...
        if (chessBoard[index + left] === blackPawn) {
          if (
            previousMove()[0] + previousMove()[1] === index + left &&
            previousMove()[1] === 16
          ) {
            moves.push(index);
            moves.push(leftUp);
      ...
      if (
        chessBoard[index + rightUp] !== emptySpace &&
        chessBoard[index + rightUp].charCodeAt() > COLORDELIMITER
      ) {
        moves.push(index);
        moves.push(rightUp);
      }
      if (
        chessBoard[index + leftUp] !== emptySpace &&
        chessBoard[index + leftUp].charCodeAt() > COLORDELIMITER
      ) {
        moves.push(index);
        moves.push(leftUp);
      }
      if (
        index <= 55 &&
        index >= 48 &&
        chessBoard[index + spaceUp * 2] === emptySpace
      ) {
        moves.push(index);
  ...
  return (
    <Draggable
      index={index}
      id={key}
      className={`${className} ${getClassNameFromIndex(index)} ${
        litUpBoxes()[index]
      }`}
    />
  );
...
function findRockMoves(chessBoard, index) {
  ...
  if (chessBoard[index] === blackRook) {
    for (let i = 1; i < 8; i++) {
      if (
        index - i >= firstOfRow &&
        (chessBoard[index - i].charCodeAt() < COLORDELIMITER ||
          chessBoard[index - i] === emptySpace)
      ) {
        moves.push(index);
        moves.push(-i);
    ...
    for (let i = 1; i < 8; i++) {
      if (
        index + i < firstOfRow + 8 &&
        (chessBoard[index + i].charCodeAt() < 91 ||
          chessBoard[index + i] === emptySpace)
      ) {
        moves.push(index);
        moves.push(i);
...
function findBishopMoves(chessBoard, index) {
  ...
  if (chessBoard[index] === blackBishop) {
    ...
    //example has index 8;
    for (let i = 7; i <= 63; i = i + 7) {
      if (index % 8 === 7 || index <= 7 || index - i < 0) {
        break;
      }
      if (
        index - i <= 63 &&
...
function findHorseMoves(chessBoard, index) {
  ...
  if (chessBoard[index] === blackKnight) {
    ...
    if (
      index >= TWOROWSFROMTOP &&
      index % ROWZISE !== LEFTCOLUMN &&
      (chessBoard[index + KUPLEFT] === emptySpace ||
        chessBoard[index + KUPLEFT].charCodeAt() < COLORDELIMITER)
    ) {
      moves.push(index);
      moves.push(KUPLEFT);
    }
    ...
    if (
      index >= TWOROWSFROMTOP &&
      index % ROWZISE !== RIGHTCOLUMN &&
      (chessBoard[index + KUPRIGHT] === emptySpace ||
        chessBoard[index + KUPRIGHT].charCodeAt() < COLORDELIMITER)
    ) {
      moves.push(index);
      moves.push(KUPRIGHT);
    }
    //DOWN AND LEFT
    if (
      index < TWOROWFROMBOTTOM &&
      index % ROWZISE !== LEFTCOLUMN &&
      (chessBoard[index + KDOWNLEFT] === emptySpace ||
        chessBoard[index + KDOWNLEFT].charCodeAt() < COLORDELIMITER)
    ) {
      // console.log(moves);
      moves.push(index);
    ...
    if (
      index < TWOROWFROMBOTTOM &&
      index % ROWZISE !== RIGHTCOLUMN &&
      (chessBoard[index + KDOWNRIGHT] === emptySpace ||
        chessBoard[index + KDOWNRIGHT].charCodeAt() < COLORDELIMITER)
    ) {
      moves.push(index);
    ...
    if (
      index >= ONEROWFROMTOP &&
      index % ROWZISE !== LEFTCOLUMN &&
      index % ROWZISE !== ONEFROMLEFT &&
      (chessBoard[index + KLEFTUP] === emptySpace ||
        chessBoard[index + KLEFTUP].charCodeAt() < COLORDELIMITER)
    ) {
      moves.push(index);
      moves.push(KLEFTUP);
    }
    //LEFT AND DOWN
    if (
      index < ONEROWFROMBOTTOM &&
      index % ROWZISE !== LEFTCOLUMN &&
      index % ROWZISE !== ONEFROMLEFT &&
      (chessBoard[index + KLEFTDOWN] === emptySpace ||
        chessBoard[index + KLEFTDOWN].charCodeAt() < COLORDELIMITER)
    ) {
      moves.push(index);
    ...
    if (
      index >= ONEROWFROMTOP &&
      index % ROWZISE !== RIGHTCOLUMN &&
      index % ROWZISE !== ONEFROMRIGHT &&
      (chessBoard[index + KRIGHTUP] === emptySpace ||
        chessBoard[index + KRIGHTUP].charCodeAt() < COLORDELIMITER)
    ) {
      moves.push(index);
      moves.push(KRIGHTUP);
    }
    ...
    if (
      ...
      index % ROWZISE !== RIGHTCOLUMN &&
      index % ROWZISE !== ONEFROMRIGHT &&
      (chessBoard[index + KIRGHTDOWN] === emptySpace ||
        chessBoard[index + KIRGHTDOWN].charCodeAt() < COLORDELIMITER)
    ) {
      moves.push(index);
...
function findKingMoves(chessBoard, index) {
  ...
  if (chessBoard[index] == blackKing) {
    ...
    if (
      chessBoard[0] == "t" &&
      chessBoard[1] == emptySpace &&
      chessBoard[2] == emptySpace &&
      chessBoard[3] == emptySpace
    ) {
      moves.push(99);
      moves.push(99);
    }
    ...
    if (
      chessBoard[7] == "t" &&
      chessBoard[5] == emptySpace &&
      chessBoard[6] == emptySpace
    ) {
      moves.push(98);
      moves.push(98);
    }
  ...
  if (chessBoard[index] == whiteKing) {
    ...
    if (
      chessBoard[56] == "T" &&
      chessBoard[59] == emptySpace &&
      chessBoard[58] == emptySpace &&
      chessBoard[57] == emptySpace
    ) {
      moves.push(97);
      moves.push(97);
    }
    if (
      chessBoard[63] == "T" &&
      chessBoard[61] == emptySpace &&
      chessBoard[62] == emptySpace
    ) {
      moves.push(96);
      moves.push(96);
    }
  ...
  if (chessBoard[index] === blackKing) {
    ...
    if (
      index + left >= firstOfRow &&
      (chessBoard[index + left].charCodeAt() < COLORDELIMITER ||
        chessBoard[index + left] === emptySpace)
    ) {
      moves.push(index);
      moves.push(left);
    }
    if (
      index + right < firstOfRow + ROWZISE &&
      (chessBoard[index + right].charCodeAt() < COLORDELIMITER ||
        chessBoard[index + right] === emptySpace)
    ) {
      moves.push(index);
      moves.push(right);
    }
    if (
      index + spaceDown <= MAXBOARDINDEX &&
      (chessBoard[index + spaceDown].charCodeAt() < COLORDELIMITER ||
        chessBoard[index + spaceDown] === emptySpace)
    ) {
      moves.push(index);
      moves.push(spaceDown);
    }
    if (
      index + spaceUp >= 0 &&
      (chessBoard[index + spaceUp].charCodeAt() < COLORDELIMITER ||
        chessBoard[index + spaceUp] === emptySpace)
    ) {
      moves.push(index);
      moves.push(spaceUp);
    }
